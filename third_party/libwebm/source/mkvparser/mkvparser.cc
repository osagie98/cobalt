...
long SeekHead::Parse() {
  ...
  while (pos < stop) {
    ...
    if (id == libwebm::kMkvSeek) {
      ++entry_count;
      if (entry_count > INT_MAX) return E_PARSE_FAILED;
    } else if (id == libwebm::kMkvVoid) {
      ++void_element_count;
      if (void_element_count > INT_MAX) return E_PARSE_FAILED;
    }
    pos += size;  // consume payload
...
bool CuePoint::Load(IMkvReader* pReader) {
  ...
  while (pos < stop) {
    ...
    if (id == libwebm::kMkvCueTime)
      ...
      if (track_positions_count > UINT_MAX) return E_PARSE_FAILED;
    }
    pos += size;  // consume payload
  }
...
long ContentEncoding::ParseContentEncodingEntry(long long start, long long size,
  ...
  while (pos < stop) {
    ...
    if (id == libwebm::kMkvContentCompression) {
      ++compression_count;
      if (compression_count > INT_MAX) return E_PARSE_FAILED;
    }
    if (id == libwebm::kMkvContentEncryption) {
      ++encryption_count;
      if (encryption_count > INT_MAX) return E_PARSE_FAILED;
    }
    pos += size;  // consume payload
    if (pos > stop) return E_FILE_FORMAT_INVALID;
  }
...
long Track::ParseContentEncodingsEntry(long long start, long long size) {
  ...
  while (pos < stop) {
    ...
    if (id == libwebm::kMkvContentEncoding) {
      ++count;
      if (count > INT_MAX) return E_PARSE_FAILED;
    }
    pos += size;  // consume payload
    if (pos > stop) return E_FILE_FORMAT_INVALID;
  }
...
bool Projection::Parse(IMkvReader* reader, long long start, long long size,
  ...
  while (read_pos < end) {
    ...
    if (child_id == libwebm::kMkvProjectionType) {
      ...
      if (projection_ptr->private_data != NULL) return false;
